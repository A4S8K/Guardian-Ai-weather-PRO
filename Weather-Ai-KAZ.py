...
# 1. [span_2](start_span)БЕТТІҢ КОНФИГУРАЦИЯСЫ (UI қабаты)[span_2](end_span)
...
with st.sidebar:
    ...
    [span_4](start_span)st.caption("Әзірлеушілер: Арслан Өмен, Иманғали Құрбанбек") #[span_4](end_span)

# 2. [span_5](start_span)[span_6](start_span)ДЕРЕКТЕРДІ ИНТЕГРАЦИЯЛАУ (API қабаты)[span_5](end_span)[span_6](end_span)
def get_coordinates(city):
    [span_7](start_span)"""Орынды нақтылау алгоритмі: Қала аты арқылы координаталарды анықтау[span_7](end_span)"""
...
def get_weather_data(lat, lon):
    [span_8](start_span)"""Метеорологиялық деректерді алу (Температура, жел, жаңбыр)[span_8](end_span)"""
...
def get_air_quality(lat, lon):
    [span_9](start_span)[span_10](start_span)"""Ауа сапасын талдау (AQI)[span_9](end_span)[span_10](end_span)"""
    url = f"https://air-quality-api.open-meteo.com/v1/air-quality?latitude={lat}&longitude={lon}&hourly=european_aqi"
    return requests.get(url).json()

# 3. [span_11](start_span)[span_12](start_span)ЖАСАНДЫ ИНТЕЛЛЕКТ ТАЛДАУЫ (Интеллект қабаты)[span_11](end_span)[span_12](end_span)
...
# 4. НЕГІЗГІ ЛОГИКА ЖӘНЕ ВИЗУАЛИЗАЦИЯ
...
if city_input:
    ...
    if location:
        ...
        # [span_14](start_span)[span_15](start_span)Деректерді өңдеу қабаты (Pandas)[span_14](end_span)[span_15](end_span)
        weather = get_weather_data(lat, lon)
        aqi_data = get_air_quality(lat, lon)
        aqi_val = aqi_data['hourly']['european_aqi'][0]
        
        # Көрсеткіштерді шығару
        ...
        with col3:
            # [span_16](start_span)Ауа сапасын талдау алгоритмі[span_16](end_span)
            status = "Жақсы" if aqi_val < 50 else "Орташа" if aqi_val < 100 else "Зиянды"
            st.metric("Ауа сапасы (AQI)", f"{aqi_val} ({status})")

        # [span_17](start_span)Апат қаупін бағалау алгоритмі[span_17](end_span)
        st.subheader("⚠️ Қауіп деңгейін бағалау")
        risk_score = 0
        if weather['hourly']['wind_speed_10m'][0] > 50: risk_score += 40  # Қатты жел
        if weather['hourly']['temperature_2m'][0] > 35: risk_score += 30  # Аптап ыстық
        if weather['hourly']['precipitation'][0] > 10: risk_score += 30   # Жауын-шашын
        
        [span_18](start_span)if risk_score >= 60: #[span_18](end_span)
            st.error(f"ЖОҒАРЫ ҚАУІП: {risk_score}% - Сақтық шараларын көріңіз!")
        else:
            st.info(f"Қауіп деңгейі төмен: {risk_score}%")

        # [span_19](start_span)[span_20](start_span)Графиктер (Visualisation)[span_19](end_span)[span_20](end_span)
        df = pd.DataFrame({
        ...
        # [span_21](start_span)[span_22](start_span)ЖИ Сараптамасы[span_21](end_span)[span_22](end_span)
        ...
        # [span_23](start_span)[span_24](start_span)Спутниктік карта (Windy API интеграциясы)[span_23](end_span)[span_24](end_span)
...
# 5. [span_25](start_span)ҚОРЫТЫНДЫ[span_25](end_span)
st.divider()
[span_26](start_span)st.caption("Жетекшісі: Н.Әсілбай. Төлеби ауданының мамандандырылған мектеп-интернаты.") #[span_26](end_span)
